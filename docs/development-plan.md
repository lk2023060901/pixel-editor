...
| UI-005 | Web 壳 | Properties Inspector | UI-001, DOM-006 | 开发中 | 已接入 Map / Layer / Object 基础 inspector 编辑与 primitive + enum + class + object reference custom properties editor，并打通 `project.propertyTypes` seed 装配、共享 UI 组件与 controller/command 更新链路；typed tiles 与更完整的 property browser 形态待补 |
...
| TSET-004 | Tileset | Tile 属性编辑 | TSET-003, DOM-006 | 开发中 | 已支持 tile class 与 primitive + enum + class + object reference custom properties 编辑；typed tiles 与更完整的 property browser 形态待补 |
| TSET-005 | Tileset | Tile Probability | TSET-004 | 未开始 | 支撑 random mode |
