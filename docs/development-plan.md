...
| EXP-002 | 导出 | JSON 导出 | EXP-001 | 已完成 | 已补齐 controller 级 `exportDocumentAsJson/exportActiveDocumentAsJson/exportActiveTilesetAsJson`，将 `File -> Export` 接到当前 map 的 `TMJ` 导出，将 `Tilesets` dock 导出按钮接到当前 tileset 的 `TSJ` 导出，并复用 `DocumentRepository` 写出而不改项目资源登记；已通过 `npm run typecheck`、`npm test`、`npm run lint`、`npm run build --workspace @pixel-editor/web` |
| EXP-003 | 导出 | Image Export | REN-003, REN-011, LYR-006 | 已完成 | 已接入 `File -> Export As Image` 的 PNG 导出链路，使用离屏 renderer 输出当前 map 图像并通过 document repository 落盘，不修改项目资源登记；已通过 `npm run typecheck`、`npm test`、`npm run lint`、`npm run build --workspace @pixel-editor/web` 验证 |
| EXP-004 | 导出 | Export Options | EXP-001, PROJ-004 | 已完成 | 已将项目级导出偏好接入 controller/exporter：`embedTilesets` 作用于 map 的 `TMX/TMJ` tileset 引用，`exportMinimized` 作用于 `TMJ/TSJ/.world` JSON 输出，`resolveObjectTypesAndProperties` 会在导出时省略 class/propertytype 元数据；当前 exporter 尚不输出 template 引用，因此 `detachTemplateInstances` 在现有导出结果上与默认行为等价；已通过 `npm run typecheck`、`npm test`、`npm run lint`、`npm run build --workspace @pixel-editor/web` 验证 |
